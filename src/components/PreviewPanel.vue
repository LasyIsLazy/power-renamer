<template>
  <div class="preview-panel panel-card">
    ...
    <div v-if="store.hasConflicts" class="conflict-banner">
      检测到 {{ store.conflictPaths.length }} 个目标路径冲突，多个文件将重命名为同一路径
    </div>
    ...
    <div v-else-if="!store.hasPreviewed" class="empty-state">
      <p>点击「预览」按钮查看重命名结果</p>
      <p class="hint">快捷键：Ctrl+Enter</p>
    </div>

    <div v-else-if="store.hasPreviewed && store.previewResults.length === 0" class="empty-state">
    ...
    <div v-if="store.hasPreviewed && store.previewResults.length > 0" class="footer">
      ...
      <button
        ...
        :disabled="store.loading || store.hasConflicts"
...
const executeConfirmMessage = computed(() => {
  ...
  const conflict = store.hasConflicts ? '\n\n警告：存在目标路径冲突！' : ''
...
const isConflict = (result) => store.conflictPaths.includes(result.new_name)
...
function requestExecute() {
  if (store.previewStale) {
    store.previewRename().then(() => {
      if (store.hasPreview && !store.hasConflicts) showExecuteConfirm.value = true
    })
    return
  }
  if (!store.hasPreview || store.hasConflicts) return
